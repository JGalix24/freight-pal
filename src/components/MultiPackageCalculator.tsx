    ...
    <div className="min-h-screen gradient-background p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-3 mb-8 animate-fade-up">
          ...
          <h1 className="font-display text-2xl md:text-3xl font-bold bg-gradient-to-r from-amber-500 to-orange-600 bg-clip-text text-transparent">
            Multi-Colis
          </h1>
        </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-6 mb-6 animate-fade-up">
          <h2 className="font-display text-lg font-semibold text-foreground mb-4">Tarifs applicables</h2>
            ...
            <div key={pkg.id} className="bg-card border border-border rounded-xl p-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-foreground">Colis #{index + 1}</span>
                </div>
                {packages.length > 1 && (
                  <Button
                    variant="ghost"
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                <div>
                  <Label className="text-xs text-muted-foreground mb-1 block">Longueur (cm)</Label>
                  <Input
                    type="number"
                    value={pkg.length}
                ...
                <div>
                  <Label className="text-xs text-muted-foreground mb-1 block">Hauteur (cm)</Label>
                  <Input
                    type="number"
                    value={pkg.height}
        ...
        <div className="flex gap-3 mt-6 animate-fade-up" style={{ animationDelay: "0.2s" }}>
          <Button
            ...
            disabled={!isValid}
            className="flex-1 bg-gradient-to-r from-amber-500 to-orange-600 text-primary-foreground hover:opacity-90 transition-opacity gap-2"
          >
          ...
          <div className="mt-8 space-y-6 animate-fade-up">
            ...
            <div className="bg-gradient-to-br from-amber-500 to-orange-600 rounded-2xl p-6 text-primary-foreground">
              <h2 className="font-display text-2xl font-bold mb-4 text-center">
                Récapitulatif - {results.totalPackages} colis
              </h2>
                ...
                {parseFloat(shipTarifCBM) > 0 && (
                  <div className="bg-primary-foreground/10 backdrop-blur rounded-xl p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-medium">Total Bateau</span>
                    </div>
                    <p className="text-sm opacity-80 mb-1">
                      Volume total: {formatNumber(results.totalVolume, 4)} m³
                    </p>
                    <p className="font-display text-3xl font-bold">
                      {formatNumber(results.totalShipCost)} {currency}
                    </p>
                  </div>
                )}
                ...
                {parseFloat(planeTarifKg) > 0 && (
                  <div className="bg-primary-foreground/10 backdrop-blur rounded-xl p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-medium">Total Avion</span>
                    </div>
                    <p className="text-sm opacity-80 mb-1">
                      Poids total: {formatNumber(results.totalWeight)} kg
                    </p>
                    <p className="font-display text-3xl font-bold">
                      {formatNumber(results.totalPlaneCost)} {currency}
                    </p>
                  </div>
                )}
              </div>

              {/* Comparison if both tarifs are set */}
                ...
                <div className="mt-4 text-center bg-primary-foreground/20 rounded-lg p-3">
                  <p className="font-medium">
                    {results.totalShipCost < results.totalPlaneCost ? (
                      <>🚢 Le bateau est moins cher de <strong>{formatNumber(results.totalPlaneCost - results.totalShipCost)} {currency}</strong></>
                    ) : (
                      <>✈️ L'avion est moins cher de <strong>{formatNumber(results.totalShipCost - results.totalPlaneCost)} {currency}</strong></>
                    )}
                  </p>
                </div>
              )}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left py-2 px-2 text-muted-foreground font-medium">Colis</th>
                      <th className="text-left py-2 px-2 text-muted-foreground font-medium">Qté</th>
                      <th className="text-left py-2 px-2 text-muted-foreground font-medium">Volume</th>
                      <th className="text-left py-2 px-2 text-muted-foreground font-medium">Poids</th>
                      {parseFloat(shipTarifCBM) > 0 && (
                        <th className="text-right py-2 px-2 text-ship font-medium">Coût Bateau</th>
                      )}
                      {parseFloat(planeTarifKg) > 0 && (
                        <th className="text-right py-2 px-2 text-plane font-medium">Coût Avion</th>
                      )}
                    </tr>
                  </thead>
                  <tbody>
                    {results.packages.map((pkg, index) => (
                      <tr key={pkg.id} className="border-b border-border/50">
                        ...
                        {parseFloat(shipTarifCBM) > 0 && (
                          <td className="py-2 px-2 text-right font-medium text-ship">
                            {formatNumber(pkg.shipCost)} {currency}
                          </td>
                        )}
                        {parseFloat(planeTarifKg) > 0 && (
                          <td className="py-2 px-2 text-right font-medium text-plane">
                            {formatNumber(pkg.planeCost)} {currency}
                          </td>
                        )}
                      </tr>
                    ))}
                    <tr className="font-bold">
                      ...
                      {parseFloat(shipTarifCBM) > 0 && (
                        <td className="py-3 px-2 text-right text-ship">
                          {formatNumber(results.totalShipCost)} {currency}
                        </td>
                      )}
                      {parseFloat(planeTarifKg) > 0 && (
                        <td className="py-3 px-2 text-right text-plane">
                          {formatNumber(results.totalPlaneCost)} {currency}
                        </td>
                      )}
                    </tr>
